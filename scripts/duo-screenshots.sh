#!/bin/bash
# Build Damp for the iPhone Duo simulator, run the unit tests there, and capture every screen.
#
...
set -o pipefail
xcodebuild build -project Damp.xcodeproj -scheme Damp -destination "$DEST" -derivedDataPath DerivedData \
...
if [ "$RUN_TESTS" = 1 ]; then
  xcodebuild test -project Damp.xcodeproj -scheme Damp -destination "$DEST" -derivedDataPath DerivedData \
    -only-testing:DampTests -skipPackagePluginValidation CODE_SIGNING_ALLOWED=NO 2>&1 | tee duo-test.log \
...
APP=$(find DerivedData/Build/Products -name "Damp.app" -maxdepth 2 | head -1)
...
SCREENS="hook drinks price reasons reveal goal first result paywall home log milestone settings share"
[ "$CI_MODE" = 1 ] && SCREENS="hook drinks price reasons reveal"
...
for s in $SCREENS; do
  ...
  for attempt in 1 2 3; do
    xcrun simctl terminate "$DEVICE" app.usedamp.damp >/dev/null 2>&1 || true; sleep 1
    xcrun simctl launch "$DEVICE" app.usedamp.damp -screenshot "$s" >/dev/null; sleep 8
