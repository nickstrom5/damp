...
final class StoreManager: ObservableObject {
    enum ProductID: String, CaseIterable {
        case yearly = "damp.yearly"
        case monthly = "damp.monthly"
        case lifetime = "damp.lifetime"
    }
    ...
    func refreshEntitlements() async {
        #if DEBUG
        // QA hook: `SIMCTL_CHILD_DAMP_FORCE_PRO=1 xcrun simctl launch …` unlocks Pro without a purchase.
        if ProcessInfo.processInfo.environment["DAMP_FORCE_PRO"] == "1" {
            isPro = true
            return
        }
        #endif
