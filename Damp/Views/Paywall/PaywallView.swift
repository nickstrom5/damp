...
struct PaywallView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Try Damp free for 7 days")
                .font(Theme.Font.title)
                .foregroundStyle(Theme.textPrimary)
            Text("About **\(Stats.money(Double(appState.answers.yearlySavingsAtGoal))) a year** back at \(appState.answers.dryNightsGoal) dry nights a week. One tap a night, a check-in that shows up, a card at every milestone.")
    ...
    private var timeline: some View {
        VStack(alignment: .leading, spacing: 0) {
            TimelineRow(symbol: "drop.fill", title: "Today", detail: "Full access. Log tonight, watch the savings.", isFirst: true)
            ...
            if ScreenshotMode.isActive {
                ...
                ForEach(store.products, id: \.id) { product in
                    if let id = StoreManager.ProductID(rawValue: product.id) {
                        PlanRow(
                            title: product.displayName.replacingOccurrences(of: "Damp ", with: ""),
                            badge: badge(for: id),
    ...
    private func sampleDetail(_ id: StoreManager.ProductID) -> String {
        switch id {
        case .yearly: return "$2.50/mo · billed $29.99/yr after 7-day free trial"
        case .monthly: return "$5.99/mo · cancel anytime"
        case .lifetime: return "$49.99 once · no subscription"
        }
    }
    ...
    private var ctaSubtitle: String? {
        if ScreenshotMode.isActive {
            switch selectedID {
            case .yearly: return "7 days free, then $29.99/year. Cancel anytime."
            case .monthly: return "$5.99/month. Cancel anytime."
            case .lifetime: return "$49.99 once. Yours forever."
            }
        }
    ...
    private func scheduleTrialReminder() {
        ...
        center.requestAuthorization(options: [.alert, .sound]) { granted, _ in
            ...
            content.title = "Your Damp trial ends in 2 days"
            content.body = "Keep the streak and the savings, or cancel in Settings. No hard feelings."
            let trigger = UNTimeIntervalNotificationTrigger(timeInterval: 5 * 24 * 60 * 60, repeats: false)
            center.add(UNNotificationRequest(identifier: "damp.trialReminder", content: content, trigger: trigger))
        }
    }
}
